import React from "react";
import {
  ChevronDown,
  Github,
  ExternalLink,
  Globe,
  Bot,
  Coffee,
} from "lucide-react";
import { Link } from "react-router-dom";

const HomePage = () => {
  const techIcons = [
    { icon: Globe, name: "Full-Stack" },
    { icon: Bot, name: "Bot Dev" },
  ];

  return (
    <div className="flex flex-col md:flex-row items-center justify-around h-[calc(100vh-5rem)] w-full">
      <div className="flex flex-col justify-center items-center h-full w-2/5">
        <h1 className="text-6xl font-bold mb-15 select-none">
          I'm{" "}
          <span className="bg-gradient-to-r from-professional-blue to-success-green dark:from-cyber-blue dark:to-matrix-green bg-clip-text text-transparent font-[UnifrakturMaguntia] tracking-wider">
            Cypher
          </span>
        </h1>

        {/* Skills Icons */}
        <div className="flex items-center gap-5">
          {techIcons.map(({ icon: Icon, name }) => (
            <div key={name} className="flex flex-col items-center">
              <span className="bg-light-gray dark:bg-card-dark p-5 rounded-xl text-professional-blue dark:text-cyber-blue shadow-hero-card hover:shadow-professional-blue dark:hover:shadow-cyber-blue transition-all duration-300">
                <Icon size={30} />
              </span>
              <span className="text-medium-gray dark:text-muted-gray text-xs mt-3 select-none">
                {name}
              </span>
            </div>
          ))}
        </div>

        {/* Buttons */}
        <div className="flex items-center justify-center gap-4 mt-10 w-full">
          <Link
            to="/projects"
            className="flex items-center gap-2 bg-success-green dark:bg-matrix-green text-white dark:text-dark-gray p-3 rounded-xl shadow-hero-card hover:shadow-success-green dark:hover:shadow-matrix-green transition-all duration-300"
          >
            <ExternalLink /> My Projects
          </Link>
          <a
            href="https://github.com/ShahabCypher"
            target="_blank"
            className="flex items-center gap-2 bg-light-gray dark:bg-dark-gray p-3 rounded-xl shadow-hero-card hover:shadow-professional-blue dark:hover:shadow-cyber-blue transition-all duration-300"
          >
            <Github /> Github
          </a>
        </div>
      </div>

      <div className="w-3/5">
        <div className="flex justify-center items-center">
          <div className="h-fit w-[90%] bg-white/5 backdrop-blur-md border border-blight dark:border-white/10 rounded-2xl p-5 hover:bg-white/10 hover:border-cyan-400/30 transition-all duration-500">
            {/* Experience Badge */}
            <div className="flex items-center gap-3 md:gap-4 mb-6 select-none">
              <div className="px-4 md:px-6 py-2 md:py-3 bg-gradient-to-r from-professional-blue/20 to-success-green/20 dark:from-cyber-blue/20 dark:to-matrix-green/20 backdrop-blur-md border border-success-green/30 dark:border-matrix-green/30 rounded-xl">
                <div className="flex items-center gap-3">
                  <div className="text-2xl md:text-3xl font-bold">5+</div>
                  <div className="text-left">
                    <div className="text-professional-blue dark:text-cyber-blue font-semibold text-sm md:text-base">
                      Years
                    </div>
                    <div className="text-xs md:text-sm text-medium-gray dark:text-white">
                      Experience
                    </div>
                  </div>
                </div>
              </div>
              <Coffee className="w-6 h-6 md:w-8 md:h-8 text-amber-400 animate-pulse" />
            </div>

            {/* About Text */}
            <div className="space-y-4 text-dark-gray dark:text-gray-300 text-sm md:text-base leading-relaxed">
              <p>
                <span className="text-professional-blue dark:text-cyber-blue font-mono">
                  // Journey started 2019
                </span>
              </p>
              <p className="pl-3">
                I discovered development through Discord's bot ecosystem in
                2019, where I built custom automation solutions for communities.
                What began as solving server management challenges evolved into
                a
                <span className="text-success-green dark:text-matrix-green font-semibold">
                  {" "}
                  comprehensive full-stack skillset
                </span>
                .
              </p>
              <p className="pl-3">
                Today, I build{" "}
                <span className="text-professional-blue dark:text-cyber-blue font-semibold">
                  scalable MERN applications
                </span>{" "}
                and deliver end-to-end solutions for businesses. My approach
                combines technical precision with user-focused design, ensuring
                every project meets both functional requirements and user
                expectations.
              </p>
              <p className="pl-3">
                I specialize in building{" "}
                <span className="text-success-green dark:text-matrix-green font-semibold">
                  modern web applications
                </span>
                ,{" "}
                <span className="text-professional-blue dark:text-cyber-blue font-semibold">
                  API integrations
                </span>
                , and custom automation tools. I build solutions ranging from
                startup MVPs to enterprise-level systems, always focusing on{" "}
                <span className="text-dark-gray dark:text-white font-semibold">
                  clean code and optimal performance
                </span>
                .
              </p>
              <p>
                <span className="text-professional-blue dark:text-cyber-blue font-mono">
                  // Always learning, always building
                </span>
              </p>
            </div>

            {/* Current Tech Stack */}
            <div className="mt-6 p-4 bg-gradient-to-r from-success-green/10 to-professional-blue/10 dark:from-cyber-blue/10 dark:to-matrix-green/10 border border-success-green/20 dark:border-matrix-green/20 rounded-xl">
              <p className="text-sm text-success-green dark:text-matrix-green flex items-center gap-2">
                <span className="w-2 h-2 bg-success-green dark:bg-matrix-green rounded-full animate-pulse"></span>
                Currently working with: MERN Stack
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default HomePage;
